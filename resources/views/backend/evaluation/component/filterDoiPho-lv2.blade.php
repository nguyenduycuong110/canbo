            ...
            <div class="action">
                <div class="uk-flex uk-flex-middle">
                    <div class="uk-search uk-flex uk-flex-middle mr10">
                        <input type="text" value="{{ request('due_date.eq') ?: old('due_date.eq') }}"  placeholder="Chọn ngày hoàn thành" name="due_date[eq]" class="datepicker start_date mr10 form-control">
                        <select name="team_id" class="form-control setupSelect2 team_vice_id team_id">
                            ...
                            @foreach($userByLevel as $record)
                                @if($record->lft > $auth->lft && $record->rgt < $auth->rgt && !in_array($record->teams->id, $teamIds))
                                    <option 
                        ...
                        <select name="user_id" class="form-control setupSelect2 user_id">
                            ...
                            @foreach($userByLevel as $record)
                                @if($record->lft > $auth->lft && $record->rgt < $auth->rgt)
                                    <option 
